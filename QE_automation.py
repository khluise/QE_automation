...
Python version of the original bash/awk script
Modified 01/2024 : fixed warning, pseudo dir, magnetic keywords
...
import shutil
import soc
import elastic_calculation as ec
import contextlib
import glob
...
class CIF2QE:
    ...
    def generate_qe_input(self, file_prefix, filename, do_ibrav=False, sg=0, home_dir=".", occupations='smearing', smearing='gaussian'):
        ...
        for atom_type in atom_types:
            ...
            # Search for UPF file in home_dir, but skip files containing 'rel'
            upf_file = f"{clean_atom_type}.upf"  # default fallback
            try:
                files_in_dir = os.listdir(home_dir)
                for file in files_in_dir:
                    if (
                        file.lower().startswith(clean_atom_type.lower())
                        and file.lower().endswith('.upf')
                        and 'rel' not in file.lower()
                    ):
                        upf_file = file
                        break
            except OSError:
    ...
    try:
        ...
        if len(energy_data) > 0:
            ...
            with open(grace_filename, 'w') as f:
                ...
                f.write("# Generated by LuiMoster.py\n")
                ...
                if not os.path.isfile(reference_input):
                    ...
                    if len(ecutwfc_values) > 1:
                        ...
                        with open(cutoff_grace_filename, 'w') as f:
                            # Write Grace header
                            f.write("# Grace project file\n")
                            f.write("# Generated by LuiMoster.py - Cutoff Energy Convergence\n")
                            f.write("@version 50123\n")
                            f.write("@page size 792, 612\n")
                            f.write("@page scroll 5%\n")
                            f.write("@page inout 5%\n")
                        ...
                        if not os.path.isfile(reference_degauss_input):
                            ...
                            if len(degauss_values) > 1:
                                ...
                                with open(degauss_grace_filename, 'w') as f:
                                    # Write Grace header
                                    f.write("# Grace project file\n")
                                    f.write("# Generated by LuiMoster.py - Degauss Convergence\n")
                                    f.write("@version 50123\n")
                                    f.write("@page size 792, 612\n")
                                    f.write("@page scroll 5%\n")
                                    f.write("@page inout 5%\n")
...
def vc_relax_run(suggested_kp_x, suggested_kp_y, suggested_kp_z, suggested_ecutwfc, suggested_degauss):
    ...
    def generate_tuned_scf_input():
        ...
        tuned_scf_input = f"{file_prefix}.tuned_scf.in"
        # write the modified input file
        with open(tuned_scf_input, 'w') as f:
            f.write(modi_content) 
    generate_tuned_scf_input()
...
def scf_run(scf_content,output_dir=''):
    if not scf_content:
        # Check if the reference input file exists
        ref_input = f"{file_prefix}.tuned_scf.in"
        if not os.path.isfile(ref_input):
            print(f"Error: Reference file {ref_input} not found!")
            print("Please make sure this k-point combination was generated in the previous step.")
        else:
        ...
        if output_dir is None:
            output_dir = input("Enter output directory (default: <file_prefix>.scf.outdir): ").strip()
        if output_dir == "":
            output_dir = f"{file_prefix}.scf"    
        
        scf_content = ref_content
        # Create new input file with modified ecutwfc and ecutrho
        scf_content = re.sub(r"outdir\s*=\s*['\"].*?['\"]", f'outdir = "{output_dir}"', scf_content)

    # Create new input and output filenames
    scf_input = f"{output_dir}.scf.in"
    scf_output = f"{output_dir}.scf.out"
...
def band_run(output_dir='' ):
    ...
    suggested_nbnd = int(2 * total_valence_electrons + 0.5)
   
    nband_value = input(f"Enter the nbnd value (suggested: {suggested_nbnd}): ").strip()
    if not nband_value:
        nband_value = str(suggested_nbnd)
    ...
    hsp_filename = f"{file_prefix}_high_sys_points.used"
    with open(hsp_filename, 'w') as f:
        f.write(hsp_input + '\n')
...
def elastic_run(output_dir=''):
    ...
    # Check if output_dir is provided, if not, use default
    if output_dir == "":
        output_dir = f"{file_prefix}.Elastic"
        
    scf_content = ref_content
    # Create new input file for scf calculation
    scf_content = re.sub(r"outdir\s*=\s*['\"].*?['\"]", f"outdir = './'", scf_content)
    scf_content = re.sub(r"calculation\s*=\s*['\"].*?['\"]", "calculation = 'scf'", scf_content)
    scf_content = re.sub(r"&SYSTEM", f"&SYSTEM\n                    lspinorb = .true.\n                    noncolin = .true.", scf_content)
    scf_input = f"{file_prefix}.scf.in"
    scf_output = f"{file_prefix}.scf.out"
    
    thermo_control_content = []
    thermo_control_content.append("&INPUT_THERMO\n")
    thermo_control_content.append(f"	what        = 'mur_lc_elastic_constants',\n")
    thermo_control_content.append(f"    frozen_ions = .false.")
    thermo_control_content.append(f"/\n")
    thermo_control_content = "".join(thermo_control_content)    
    thermo_control_input = f"thermo_control"
    ...
    try:
        ...
        # write the thermo control input file
        with open(thermo_control_input, 'w') as f:
            f.write(thermo_control_content)
        print(f"\nGenerated {thermo_control_input}.")
        
        # Run the elastic constants calculation
        print(f"Running thermo_pw.x using: {scf_input} and {thermo_control_input}")
        success = run_elastic_calculation(scf_input, scf_output, num_cores) 
        if not success:
            print(f"\n✗ FATAL ERROR: Calculation failed for {scf_input}")
            print("The thermo_pw.x calculation did not complete successfully.")
            print("Please check:")
            print("- Input file syntax and parameters")
            print("- Pseudopotential files availability")
            print("- System resources (memory, disk space)")
        ...
        # Calculate the properties from the output file
        elastic_properties_file = f"{file_prefix}.elastic_properties.out"
        with open(elastic_properties_file, 'w') as f:
            with contextlib.redirect_stdout(f):
                ec.calculate(scf_input, scf_output, file_prefix)
...
if __name__ == "__main__":
    ...
    description='CIF to Quantum Espresso format converter',
    formatter_class=argparse.RawDescriptionHelpFormatter,
    epilog="""
    Examples:
    python LuiMoster.py
    python LuiMoster.py -s     (uses CRYSTAL sg atom position mode)
    """
)

    # Set ibrav to always True by default, so user does not need to specify -i
    parser.add_argument('-i', '--ibrav', action='store_true', 
                    help='Use the ibrav of QE (default: True)')
    parser.add_argument('-s', '--crystal', action='store_true',
                    help='Use CRYSTAL sg atom position mode')
    parser.add_argument('--version', action='version', version='LuiMoster.py Version 1.0')
    
    args = parser.parse_args()
    args.ibrav = True  # Force ibrav to always be True
    
    # Ask user for CIF file input
    print("=" * 40)
    print("Quantum Espresso Automation Script by \033[94mLuise\033[0m")
    print("=" * 40)
    
    while True:
        cif_input = input("Enter CIF filename (with or without .cif extension): ").strip()
        
        if not cif_input:
            print("Please enter a filename.")
            continue
        
        global file_prefix, filename  ### Can be problematic but need for soc calculation
        # Process filename
        if cif_input.endswith('.cif'):
            
            file_prefix = cif_input[:-4]
            filename = cif_input
        else:
            file_prefix = cif_input
            filename = cif_input + '.cif'
        ...
        if os.path.isfile(filename):
            ...
            print(f"Error: Cannot find file '{filename}'. Please check the filename and try again.")
            continue
    
    # Set options
    do_ibrav = args.ibrav
    sg = 2 if args.crystal else 0
    home_dir = os.getcwd()
    
    # Ask user for number of CPU cores for MPI run 
    try:
        num_cores = int(input("Enter the number of CPU cores for MPI run: "))
        if num_cores <= 0:
            print("Error: Number of cores must be positive")
            sys.exit(1)
    except ValueError:
        print("Error: Please enter a valid number")
        sys.exit(1)
    print(f"Using {num_cores} CPU cores for the calculation\n")
    
    soc.file_prefix = file_prefix  # Set the file_prefix for SOC calculations
    soc.filename = filename  # Set the filename for SOC calculations
    soc.num_cores = num_cores  # Set the number of cores for SOC calculations
    soc.sg = sg  # Set the space group for SOC calculations
    soc.do_ibrav = do_ibrav  # Set the ibrav flag for SOC calculations
    soc.home_dir = home_dir  # Set the home directory for SOC calculations
    
    
    ###===========================================================================================================###
    ###                                  Edit Below for your specific calculations                                ###
    ###===========================================================================================================###
    ###                                  Uncomment the functions you want to run                                  ###
    ###===========================================================================================================###
    ###                       Get_tuned_scf() must be run at least before any other calculations                  ###
    ###===========================================================================================================### 
    ###         Similarly for soc calculations soc.get_tuned_scf() must be run before any other calculations      ###
    ###===========================================================================================================###
